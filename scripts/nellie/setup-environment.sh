...
app_dir=$(pwd)
target_path="$app_dir/extensions/nellie-submodule/extensions/vscode"
link_path="$app_dir/extensions/nellie-ref"

# Run the base functionality
echo -e "\nInitializing sub-modules..."

# Check if the submodule directory already exists
if [ -d "$app_dir/extensions/nellie-submodule" ]; then
    echo "Removing existing nellie-submodule directory"
    execute "rm -rf $app_dir/extensions/nellie-submodule" "Failed to remove existing nellie-submodule directory"
fi

# Check if the Nellie IDE Roo Code directory already exists
if [ -d "$app_dir/extensions/Nellie IDE-Roo-Code" ]; then
    echo "Removing existing Nellie IDE-Roo-Code directory"
    execute "rm -rf $app_dir/extensions/Nellie IDE-Roo-Code" "Failed to remove existing Nellie IDE-Roo-Code directory"
fi
...
execute "cd ./extensions/nellie-submodule" "Failed to change directory to extensions/nellie-submodule"
...
# ROO CODE
execute "cd ../Nellie IDE-Roo-Code" "Failed to change directory to extensions/Nellie IDE-Roo-Code"
# Discard any potential changes or merge conflicts in the working directory or staging area,
# ensuring local branch matches remote branch exactly before checking out main
execute "git reset --hard" "Failed to reset --hard"
execute "git checkout main" "Failed to checkout main branch"
...
# Make sure the submodule has the latest updates
execute "git pull origin main" "Failed to pull latest changes from origin/main"
execute "npm run install:all" "Failed to install dependencies for the Nellie IDE-Roo-Code"
execute "npm run build" "Failed to build the Nellie IDE-Roo-Code"
