...
function toEntries(themes: Array<IWorkbenchTheme>, category?: string, showOnlyNellie: boolean = true): QuickPickInput<ThemeItem>[] {
	...
	// Nellie IDE filtering logic
	const nellieThemes = themes.filter(t => t.label.includes('Nellie IDE'));
	const otherThemes = themes.filter(t => !t.label.includes('Nellie IDE'));

	const entries: QuickPickInput<ThemeItem>[] = [];

	if (nellieThemes.length > 0) {
		entries.push({ type: 'separator', label: category ? category : 'Nellie IDE Themes' });
		entries.push(...nellieThemes.map(toEntry).sort(sorter));
	}
...
class DefaultThemeUpdatedNotificationContribution implements IWorkbenchContribution {
	...
	private async _showYouGotMigratedNotification(): Promise<void> {
		...
		if (newTheme) {
			...
			await this._notificationService.prompt(
				Severity.Info,
				localize({ key: 'themeUpdatedNotification', comment: ['{0} is the name of the new default theme'] }, "Nellie IDE now ships with a new default theme '{0}'. If you prefer, you can switch back to the old theme or try one of the many other color themes available.", newTheme.label),
				choices,
				{
	...
	private async _tryNewThemeNotification(): Promise<void> {
		...
		if (theme) {
			...
			await this._notificationService.prompt(
				Severity.Info,
				localize({ key: 'newThemeNotification', comment: ['{0} is the name of the new default theme'] }, "Nellie IDE now ships with a new default theme '{0}'. Do you want to give it a try?", theme.label),
				choices,
				{ onCancel: () => this._writeTelemetry('cancel') }
			);
		}
