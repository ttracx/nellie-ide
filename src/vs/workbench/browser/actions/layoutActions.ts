...
import { AuxiliaryBarVisibleContext, PanelAlignmentContext, PanelVisibleContext, SideBarVisibleContext, FocusedViewContext, InEditorZenModeContext, IsMainEditorCenteredLayoutContext, MainEditorAreaVisibleContext, IsMainWindowFullscreenContext, PanelPositionContext, IsAuxiliaryWindowFocusedContext, TitleBarStyleContext, NellieVisibleContext, NellieCreatorVisibleContext } from '../../common/contextkeys.js';
...
import { ToggleNellieOverlayAction } from '../parts/overlay/nellieOverlayActions.js';
...
const nellieIcon = registerIcon('nellie', Codicon.archive, localize('nellieIcon', "Represents the Nellie IDE overlay"));
...
MenuRegistry.appendMenuItems([{
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar left', "Move Nellie IDE Side Bar Left")
		},
	...
	item: {
		...
		command: {
			id: ToggleSidebarPositionAction.ID,
			title: localize('move second sidebar right', "Move Nellie IDE Side Bar Right")
		},
...
registerAction2(class extends Action2 {
	...
	run(accessor: ServicesAccessor): void {
		...
		commandService.executeCommand('workbench.action.moveNellieExtensionToAuxBar');
	}
});
			...
			containerModel.visibleViewDescriptors.forEach(viewDescriptor => {
				if (viewDescriptor.canMoveView) {
					if (!hasAddedView) {
						results.push({
							type: 'separator',
							label: localize('secondarySideBarContainer', "Nellie IDE Side Bar / {0}", containerModel.title)
						});
						hasAddedView = true;
					}
	...
	run(accessor: ServicesAccessor, viewId?: string): void {
		...
		if (!(isViewSolo && currentLocation === ViewContainerLocation.AuxiliaryBar)) {
			items.push({
				...
				label: localize('moveFocusedView.newContainerInSidePanel', "New Nellie IDE Side Bar Entry")
			});
		}
		...
		items.push({
			type: 'separator',
			label: localize('secondarySideBar', "Nellie IDE Side Bar")
		});
...
ToggleVisibilityActions.push(...[
	...
	CreateToggleLayoutItem(ToggleAuxiliaryBarAction.ID, AuxiliaryBarVisibleContext, localize('secondarySideBar', "Nellie IDE Side Bar"), { whenA: ContextKeyExpr.equals('config.workbench.sideBar.location', 'left'), iconA: panelRightIcon, iconB: panelLeftIcon }),
	CreateToggleLayoutItem(TogglePanelAction.ID, PanelVisibleContext, localize('panel', "Panel"), panelIcon),
	CreateToggleLayoutItem(ToggleStatusbarVisibilityAction.ID, ContextKeyExpr.equals('config.workbench.statusBar.visible', true), localize('statusBar', "Status Bar"), statusBarIcon),
	CreateToggleLayoutItem(ToggleNellieOverlayAction.ID, NellieVisibleContext, 'Nellie IDE', nellieIcon),
	CreateToggleLayoutItem(ToggleCreatorOverlayAction.ID, NellieCreatorVisibleContext, 'Nellie IDE Creator'),
]);
...
registerAction2(class NellieSettingsAction extends Action2 {
	constructor() {
		super({
			id: 'workbench.action.nellie-ideSettings',
			title: localize2('nellieSettings', "Nellie IDE Settings"),
			f1: true,
	...
	run(accessor: ServicesAccessor): void {
		const commandService = accessor.get(ICommandService);
		commandService.executeCommand("nellie.toggleOverlay")
	}
});
