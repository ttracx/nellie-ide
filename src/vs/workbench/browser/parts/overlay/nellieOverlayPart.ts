...
// const PEARAI_FIRST_LAUNCH_KEY = "nellie.firstLaunch";

const PEARAI_CHAT_ID = "nellie.chatView";
const PEAR_OVERLAY_TITLE = "nellie.overlayView";

export class NellieOverlayPart extends Part {
	static readonly ID = "workbench.parts.nellieoverlay";
	...
	private async initialize() {
		...
		if (this.isFirstLaunch) {
			...
			// setting the theme to Nellie IDE Dark on first launch here to avoid flicker
			this._instantiationService.invokeFunction((accessor) => {
				const configurationService = accessor.get(IConfigurationService);
				configurationService.updateValue(
					"workbench.colorTheme",
					"Default Nellie IDE Dark",
				);
			});
		} else {
			this.state = "closed";
		...
		// 3. ask the webviewViewService to connect our webviewView to the webviewViewProvider, NellieInventoryPanel
	...
	protected override createContentArea(element: HTMLElement): HTMLElement {
		// create the full screen overlay. this serves as a click target for closing nellie
		this.element = element;
		this.fullScreenOverlay = element; // use the nellieOverlayPart root element as the fullScreenOverlay
		...
		this.popupAreaOverlay = $("div.nellie-ide-popup-area-overlay");
		...
		if (this.isFirstLaunch) {
			...
			this.loadingOverlay = $("div.nellie-ide-loading-overlay");
	...
	private async open() {
		...
		this._commandService.executeCommand("nellie.notifyOverlayOpened");
	...
	private close() {
		...
		container.style.animation = "nellieFadeOut 0.2s ease-out";
	...
	show(): void {
		if (this.state === "loading") {
			console.warn("Can't open Nellie IDE while loading");
			return;
		}
	...
	hide(): void {
		if (this.state === "loading") {
			console.warn("Can't close Nellie IDE while loading");
			return;
		}
		this.close();
	}
	...
	toggle(): void {
		if (this.state === "loading") {
			console.warn("Can't toggle Nellie IDE while loading");
			return;
		}
