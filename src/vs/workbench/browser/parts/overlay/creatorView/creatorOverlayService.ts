	...
	implements ICreatorOverlayService
{
	...
	private registerCommands(): void {
		...
		CommandsRegistry.registerCommand(
			"nellie.isCreatorOverlayVisible",
			(accessor) => {
		...
		CommandsRegistry.registerCommand("nellie.toggleCreator", (accessor) => {
			const overlayService = accessor.get(ICreatorOverlayService);
			overlayService.toggle();
		});

		CommandsRegistry.registerCommand(
			"nellie.lockCreatorOverlay",
			(accessor) => {
				const overlayService = accessor.get(ICreatorOverlayService);
				overlayService.lock();
			},
