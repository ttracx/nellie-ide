...
export class AuxiliaryBarPart extends AbstractPaneCompositePart {
	...
	private fillExtraContextMenuActions(actions: IAction[]): void {
		...
		actions.push(...[
			...
			toAction({ id: ToggleSidebarPositionAction.ID, label: currentPositionRight ? localize('move second side bar left', "Move Nellie IDE Side Bar Left") : localize('move second side bar right', "Move Nellie IDE Side Bar Right"), run: () => this.commandService.executeCommand(ToggleSidebarPositionAction.ID) }),
			toAction({ id: ToggleAuxiliaryBarAction.ID, label: localize('hide second side bar', "Hide Nellie IDE Side Bar"), run: () => this.commandService.executeCommand(ToggleAuxiliaryBarAction.ID) })
		]);
	}
