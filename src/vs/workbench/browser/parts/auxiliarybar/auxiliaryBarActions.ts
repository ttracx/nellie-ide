...
export class ToggleAuxiliaryBarAction extends Action2 {
	...
	static readonly LABEL = localize2('toggleAuxiliaryBar', "Toggle Nellie IDE Side Bar Visibility");
	...
	constructor() {
		super({
			...
			toggled: {
				...
				title: localize('secondary sidebar', "Nellie IDE Side Bar"),
...
registerAction2(class FocusAuxiliaryBarAction extends Action2 {
	...
	static readonly LABEL = localize2('focusAuxiliaryBar', "Focus into Nellie IDE Side Bar");
...
MenuRegistry.appendMenuItems([
	{
		...
		item: {
			...
			command: {
				id: ToggleAuxiliaryBarAction.ID,
				title: localize('toggleSecondarySideBar', "Toggle Nellie IDE Side Bar"),
		...
		item: {
			...
			command: {
				id: ToggleAuxiliaryBarAction.ID,
				title: localize2('hideAuxiliaryBar', 'Hide Nellie IDE Side Bar'),
			},
...
registerAction2(class extends SwitchCompositeViewAction {
	constructor() {
		super({
			...
			title: localize2('previousAuxiliaryBarView', 'Previous Nellie IDE Side Bar View'),
			category: Categories.View,
			f1: true
...
registerAction2(class extends SwitchCompositeViewAction {
	constructor() {
		super({
			...
			title: localize2('nextAuxiliaryBarView', 'Next Nellie IDE Side Bar View'),
			category: Categories.View,
			f1: true
		}, ViewContainerLocation.AuxiliaryBar, 1);
	}
...
class FocusNellieExtensionAction extends Action2 {
	static readonly ID = 'workbench.action.focusNellieExtension';
	static readonly LABEL = localize2(
		"focusNellieExtension",
		"Focus into Nellie IDE Extension",
	);

	constructor() {
		super({
			id: FocusNellieExtensionAction.ID,
			title: FocusNellieExtensionAction.LABEL,
			category: Categories.View,
			f1: true,
			// keybinding: do not add keybinding CTRL/CMD L here, it comes from nellie extension
		});
	}

	override async run(accessor: ServicesAccessor): Promise<void> {
		// focus nellie extension
		const commandService = accessor.get(ICommandService);
		commandService.executeCommand('nellie.focusContinueInput');
	}
}

registerAction2(FocusNellieExtensionAction);

// Following is a only Nellie IDE related action, need to refactor these type of actions to separate file

class OpenNellieDocsAction extends Action2 {
	static readonly ID = 'workbench.action.openNellieDocs';
	static readonly LABEL = localize2(
		"openNellieDocs",
		"Open Nellie IDE Documentation",
	);

	constructor() {
		super({
			id: OpenNellieDocsAction.ID,
			title: OpenNellieDocsAction.LABEL,
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		if (!productService.nellieLinks?.docs) {
			return;
		}
		await openerService.open(URI.parse(productService.nellieLinks?.docs));
	}
}

registerAction2(OpenNellieDocsAction);
