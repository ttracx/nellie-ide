...
import { NellieChatExtensionId, NellieSearchExtensionId, NellieMemoryExtensionId, NellieRooExtensionId, NellieView, NELLIE_VIEWS} from '../../../services/views/nellie/nellieViewsShared.js';
...
export class SwitchToNellieIntegrationAction extends Action2 {
  static readonly ID = 'workbench.action.switchToNellieIntegrationIconBar';

  constructor() {
    super({
      id: SwitchToNellieIntegrationAction.ID,
    title: localize2('switchToNellieView', "Switch to Nellie IDE Integration"),
    category: Categories.View,
    f1: true
    });
  }

  override async run(accessor: ServicesAccessor, args?: { view: NellieView }): Promise<void> {
    ...
    await viewsService.openView(NELLIE_VIEWS[view], true);
  }
}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: localize2('closeSecondarySideBar', 'Hide Nellie IDE Side Bar'),
			category: Categories.View,
			icon: closeIcon,
			menu: [{
				id: MenuId.CommandPalette,
...
// Move Nellie IDE extension to Nellie IDE Side Bar (Auxiliary Bar) (we want Nellie IDE Side Bar to be default loaction for extension)
class MoveNellieExtensionToAuxBarAction extends MoveViewsBetweenPanelsAction {
    static readonly ID = 'workbench.action.moveNellieExtensionToAuxBar';

    constructor() {
        super(ViewContainerLocation.Sidebar, ViewContainerLocation.AuxiliaryBar, {
            id: MoveNellieExtensionToAuxBarAction.ID,
            title: localize2('moveNellieExtensionToAuxBar', "Move Nellie Extension to Auxiliary Bar"),
            category: Categories.View,
            f1: true
        });
    }
    ...
    override run(accessor: ServicesAccessor): void {
        ...
        const chatViewContainer = viewDescriptorService.getViewContainerById(NellieChatExtensionId);
        const searchViewContainer = viewDescriptorService.getViewContainerById(NellieSearchExtensionId);
        const memoryViewContainer = viewDescriptorService.getViewContainerById(NellieMemoryExtensionId);
        const agentViewContainer = viewDescriptorService.getViewContainerById(NellieRooExtensionId);
...
registerAction2(MoveNellieExtensionToAuxBarAction);

// --- Move Panel Views To Nellie IDE Side Bar
...
class MovePanelToSidePanelAction extends MoveViewsBetweenPanelsAction {
	...
	constructor() {
		super(ViewContainerLocation.Panel, ViewContainerLocation.AuxiliaryBar, {
			...
			title: localize2('movePanelToSecondarySideBar', "Move Panel Views To Nellie IDE Side Bar"),
			category: Categories.View,
			f1: false
		});
	}
...
// --- Move Nellie IDE Side Bar Views To Panel
...
class MoveSidePanelToPanelAction extends MoveViewsBetweenPanelsAction {
	...
	constructor() {
		super(ViewContainerLocation.AuxiliaryBar, ViewContainerLocation.Panel, {
			...
			title: localize2('moveSidePanelToPanel', "Move Nellie IDE Side Bar Views To Panel"),
			category: Categories.View,
			f1: false
		});
	}
