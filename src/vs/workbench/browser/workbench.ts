...
import { INellieOverlayService } from '../browser/parts/overlay/nellieOverlayService.js';
...
export class Workbench extends Layout {
	...
	private renderWorkbench(instantiationService: IInstantiationService, notificationService: NotificationService, storageService: IStorageService, configurationService: IConfigurationService): void {
		...
		for (const { id, role, classes, options } of [
			...
			{ id: Parts.NELLIEOVERLAY_PART, role: 'none', classes: [] },
			{ id: Parts.NELLIECREATOROVERLAY_PART, role: 'none', classes: [] },
		]) {
			const partContainer = this.createPart(id, role, classes);

			if (id === Parts.NELLIEOVERLAY_PART) {
				instantiationService.invokeFunction(accessor => {
					accessor.get(INellieOverlayService);
				});
			}

			if(id === Parts.NELLIECREATOROVERLAY_PART) {
				instantiationService.invokeFunction(accessor => {
